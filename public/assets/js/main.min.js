...
offset:function(options){if(arguments.length)return void 0===options?this:this.each(function(i){jQuery.offset.setOffset(this,options,i)});var docElem,win,rect,doc,elem=this[0];if(elem)return elem.getClientRects().length?(rect=elem.getBoundingClientRect(),rect.width||rect.height?(doc=elem.ownerDocument,win=getWindow(doc),docElem=doc.documentElement,{top:rect.top+win.pageYOffset-docElem.clientTop,left:rect.left+win.pageXOffset-docElem.clientLeft}):rect):{top:0,left:0}},position:function(){if(this[0]){var offsetParent,offset,elem=this[0],parentOffset={top:0,left:0};return"fixed"===jQuery.css(elem,"position")?offset=elem.getBoundingClientRect():(offsetParent=this.offsetParent(),offset=this.offset(),jQuery.nodeName(offsetParent[0],"html")||(parentOffset=offsetParent.offset()),parentOffset={top:parentOffset.top+jQuery.css(offsetParent[0],"borderTopWidth",!0),left:parentOffset.left+jQuery.css(offsetParent[0],"borderLeftWidth",!0)}),{top:offset.top-parentOffset.top-jQuery.css(elem,"marginTop",!0),left:offset.left-parentOffset.left-jQuery.css(elem,"marginLeft",!0)}}},offsetParent:function(){return this.map(function(){for(var offsetParent=this.offsetParent;offsetParent&&"static"===jQuery.css(offsetParent,"position");)offsetParent=offsetParent.offsetParent;return offsetParent||documentElement})}}),jQuery.each({scrollLeft:"pageXOffset",scrollTop:"pageYOffset"},function(method,prop){var top="pageYOffset"===prop;jQuery.fn[method]=function(val){return access(this,function(elem,method,val){var win=getWindow(elem);return void 0===val?win?win[prop]:elem[method]:void(win?win.scrollTo(top?win.pageXOffset:val,top?val:win.pageYOffset):elem[method]=val)},method,val,arguments.length)}}),jQuery.each(["top","left"],function(i,prop){jQuery.cssHooks[prop]=addGetHookIf(support.pixelPosition,function(elem,computed){if(computed)return computed=curCSS(elem,prop),rnumnonpx.test(computed)?jQuery(elem).position()[prop]+"px":computed})}),jQuery.each({Height:"height",Width:"width"},function(name,type){jQuery.each({padding:"inner"+name,content:type,"":"outer"+name},function(defaultExtra,funcName){jQuery.fn[funcName]=function(margin,value){var chainable=arguments.length&&(defaultExtra||"boolean"!=typeof margin),extra=defaultExtra||(margin===!0||value===!0?"margin":"border");return access(this,function(elem,type,value){var doc;return jQuery.isWindow(elem)?0===funcName.indexOf("outer")?elem["inner"+name]:elem.document.documentElement["client"+name]:9===elem.nodeType?(doc=elem.documentElement,Math.max(elem.body["scroll"+name],doc["scroll"+name],elem.body["offset"+name],doc["offset"+name],doc["client"+name])):void 0===value?jQuery.css(elem,type,extra):jQuery.style(elem,type,value,extra)},type,chainable?margin:void 0,chainable)}})}),jQuery.fn.extend({bind:function(types,data,fn){return this.on(types,null,data,fn)},unbind:function(types,fn){return this.off(types,null,fn)},delegate:function(selector,types,data,fn){return this.on(types,selector,data,fn)},undelegate:function(selector,types,fn){return 1===arguments.length?this.off(selector,"**"):this.off(types,selector||"**",fn)}}),jQuery.parseJSON=JSON.parse,"function"==typeof define&&define.amd&&define("jquery",[],function(){return jQuery});var _jQuery=window.jQuery,_$=window.$;return jQuery.noConflict=function(deep){return window.$===jQuery&&(window.$=_$),deep&&window.jQuery===jQuery&&(window.jQuery=_jQuery),jQuery},noGlobal||(window.jQuery=window.$=jQuery),jQuery}),$(function(){function updateItensCart(){var itens=JSON.parse(localStorage.productsCart).length;$(".items-count").html(itens)}function setLocalStorage(){localStorage.productsCart||(localStorage.productsCart=JSON.stringify([]))}function saveProductToLocalStorage(product){var productsCart=[];productsCart=JSON.parse(localStorage.getItem("productsCart")),productsCart.push(product),localStorage.setItem("productsCart",JSON.stringify(productsCart)),updatePayment()}function removeProductFromLocalStorage(productSku){for(var storedProducts=JSON.parse(localStorage.productsCart),i=0;i<storedProducts.length;i++){var data=parseInt(storedProducts[i].sku),sku=parseInt(productSku);data===sku&&storedProducts.splice(i,1)}localStorage.setItem("productsCart",JSON.stringify(storedProducts)),updatePayment(),updateItensCart()}function updatePayment(){for(var subtotal=0,simbol="",installmentPrice=0,storedPrductsJSON=localStorage.productsCart||"[]",storedProducts=JSON.parse(storedPrductsJSON),i=0;i<storedProducts.length;i++)subtotal+=storedProducts[i].price,simbol=storedProducts[i].currencyFormat;installmentPrice=subtotal/10,0===subtotal?($(".cart-total strong").html("SACOLA VAZIA"),$(".price-total ").css("display","none")):($(".cart-total strong").html("SUBTOTAL"),$(".price-total").css("display","block")),$(".price-total strong").html(simbol+" "+subtotal.toFixed(2).split(".")),$(".price-total span").html("ou em até 10 X R$ "+installmentPrice.toFixed(2).split("."))}function cart(){var template="";if(storageData){$.each(storageData,function(index,product){template+='<div class="list-product-cart">',template+='\t<div class="content-cart">',template+='\t\t<div class="image-product">',template+='\t\t\t<img src="assets/images/'+product.mainImage+'" alt="'+product.title+'" />',template+="\t\t</div>",template+='\t\t<div class="description-product">',template+="\t\t\t<strong>"+product.title+"</strong>",template+="\t\t\t<p>"+product.availableSizes[1]+" | "+product.style+"</p>",template+="\t\t\t<p>Quantidade: 1</p>",template+="\t\t</div>",template+='\t\t<span class="glyphicon glyphicon-remove remove-product" data-sku="'+product.sku+'" aria-hidden="true"></span>',template+='\t\t<div class="price-product">',template+="\t\t\t<strong>"+product.currencyFormat+product.price.toFixed(2).split(".")+"</strong>",template+="\t\t</div>",template+="\t</div>",template+="</div>"}),$(template).appendTo(".box-content-cart")}}function isTouchDevice(){return!0===("ontouchstart"in window||window.DocumentTouch&&document instanceof DocumentTouch)}var url="http://localhost:3000/products",products={},storageData=JSON.parse(localStorage.productsCart||"[]");setLocalStorage(),cart(),updatePayment(),updateItensCart(),$.ajax({url:url,dataType:"json",success:function(data){var template="";products=data,$.each(products,function(index,product){installmentPrice=product.price/product.installments,template+='<div class="col-lg-4 col-md-4 col-sm-4 col-xs-12 animate fadeIn" data-sku="'+product.sku+'">',template+='\t<div class="products animate fadeIn" data-sku="'+product.sku+'">',template+='\t\t<img src="assets/images/'+product.mainImage+'" alt="" />',template+="\t\t<h2>"+product.title+"</h2>",template+="\t\t<hr>",template+='\t\t<div class="price">',template+='\t\t\t<span class="full-value">'+product.currencyFormat+" <strong>"+product.price.toFixed(2).split(".")[0]+"</strong>, "+product.price.toFixed(2).split(".")[1]+"</span>",template+='\t\t\t<span class="installment">ou '+product.installments+"x de "+product.currencyFormat+" "+installmentPrice.toFixed(2).split(".")+"</span>",template+="\t\t</div>",template+="\t</div>",template+="</div>"}),$(".js-content-products").html(template)}}),$(".js-content-products").delegate(".products","click",function(e){e.preventDefault();var sku=$(this).attr("data-sku"),template="";$.each(products,function(index,product){product.sku==sku&&(template+='<div class="list-product-cart">',template+='\t<div class="content-cart">',template+='\t\t<div class="image-product">',template+='\t\t\t<img src="assets/images/'+product.mainImage+'" alt="'+product.title+'" />',template+="\t\t</div>",template+='\t\t<div class="description-product">',template+="\t\t\t<strong>"+product.title+"</strong>",template+="\t\t\t<p>"+product.availableSizes[1]+" | "+product.style+"</p>",template+="\t\t\t<p>Quantidade: 1</p>",template+="\t\t</div>",template+='\t\t<span class="glyphicon glyphicon-remove remove-product" data-sku="'+product.sku+'" aria-hidden="true"></span>',template+='\t\t<div class="price-product">',template+="\t\t\t<strong>"+product.currencyFormat+product.price.toFixed(2).split(".")+"</strong>",template+="\t\t</div>",template+="\t</div>",template+="</div>",saveProductToLocalStorage(product))}),$(template).appendTo(".box-content-cart"),$(".items-count").addClass("animated rubberBand"),setTimeout(function(){$(".items-count").removeClass("animated rubberBand")},2e3),updateItensCart()}),$(".box-content-cart").delegate(".remove-product","click",function(e){e.stopPropagation(),$(this).parents(".list-product-cart").remove();var sku=$(this).attr("data-sku");removeProductFromLocalStorage(sku)}),isTouchDevice()===!1&&$(".bag-icon").tooltip(),$(".open-cart").on("click",function(){document.getElementById("theCart").style.display="block",updatePayment()}),$(".close-cart").on("click",function(){document.getElementById("theCart").style.display="none"})});